...
export default function Dispatcher({ user, onLogout }) {
  ...
  // FIX: useRef so parsePaste always reads FRESH form (no stale closure bug)
  ...
  // ── LABELED mode: each field has a label before colon ──
  function parseLabeledText(lines, currentForm) {
    // FIX: use RegExp constructor with proper escaping (no template literal \s bug)
    const get = (...keys) => {
    ...
    const notes = get(
      "ISSUE", "ISSUE/AMOUNT COLLECTION", "ISSUE/AMMOUNT COLLECTION",
      "AMOUNT COLLECTION", "AMMOUNT COLLECTION", "NOTES", "CONCERN", "PROBLEM", "REPORT"
    );
    ...
    const cl = get("NAME","CLIENT","CLIENT NAME","SUBSCRIBER");
    const ct = get("CONTACT","CONTACT NUMBER","CONTACT NO","PHONE","MOBILE","NUMBER");
    const ad = get("ADDRESS","ADD","ADDR","LOCATION");
    if (cl) { newForm.client=cl; filled.client=true; }
    if (ct) { newForm.contact=ct; filled.contact=true; }
    if (ad) { newForm.address=ad; filled.address=true; }
  ...
  // ── FREE mode: no labels, auto-detect each line ──
  function parseFreeText(lines, currentForm) {
    ...
    for (const line of lines) {
      ...
      // LCP/NAP/PORT: "L1 N2 P14", "L1/N2/P14", "1 2 14"
      ...
      if (isLNPLabeled || isLNPNumeric) {
        ...
        if (r.lcp||r.nap||r.port) {
          lcp=r.lcp; nap=r.nap; port=r.port;
          filled.lcp=!!r.lcp; filled.nap=!!r.nap; filled.port=!!r.port;
          continue;
        }
      }
      ...
      // Amount
      if (/^[₱]?\d[\d,.]*$/.test(l)) {
        amountToCollect=l.replace(/[₱,\s]/g,""); filled.amountToCollect=true; continue;
      }

      remaining.push(l);
    }

    const addressKw = /blk|lot|phase|brgy|barangay|st\.|street|ave|road|purok|sitio|village|subd|subdivision/i;
    for (const l of remaining) {
      if (!client && l.split(" ").length<=5 && !/\d/.test(l) && !addressKw.test(l)) {
        client=l; filled.client=true;
      } else if (!address && (addressKw.test(l)||(client&&!address&&!notes))) {
        address=l; filled.address=true;
      } else if (!notes) {
        notes=l; filled.notes=true;
      }
    }
  ...
  // FIX: reads formRef.current — always fresh, never stale
  ...
  function resetPaste() {
    setPasteText(""); setParsedOk(false); setParsedFields({}); setParseMode("labeled");
  }
  ...
  async function deleteTechnician(techId) {
    ...
    Object.entries(jobs).forEach(([jobId,job])=>{
      const cur=job.techIds||[];
      if (!cur.includes(techId)) return;
        ...
        <div style={s.tbRight}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{display:"flex",flexDirection:"column"}}>
              ...
              <span style={{fontSize:10,color:"#7b87b8"}}>{user?.isCsr?"CSR":"Dispatcher"}</span>
            </div>
        ...
        <main style={{...s.content,padding:isMobile?10:20}}>
              ...
              <div style={s.card}>
                <div style={s.toolbar}>
                  <div style={{display:"flex",flexDirection:"column",gap:8,flex:1}}>
                    ...
                    <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
                      <span style={s.filterLabel}>Task:</span>
                    ...
                    <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
                      ...
                      {dateFilter==="specific"&&<input type="date" value={specificDate} onChange={e=>setSpecificDate(e.target.value)} style={s.dateInput}/>}
                      {dateFilter==="month"&&<input type="month" value={specificMonth} onChange={e=>setSpecificMonth(e.target.value)} style={{...s.dateInput,borderColor:"#9b78f5"}}/>}
                    </div>
                  </div>
          ...
          {page==="dispatch"&&(
            <div>
              ...
              <div style={s.srow}>
                {[["Kailangan ng Dispatch",counts.pending,"#f05555"],["Available Techs",Object.keys(techs).length,"#2dcc7a"],["Active",jobList.filter(([,j])=>!["done","cancelled"].includes(j.status)).length,"#4d8ef5"],["Done",counts.done,"#20c8b0"]].map(([lbl,val,col])=>(
                  <div key={lbl} style={s.sc}><div style={{...s.scTop,background:col}}/><div style={s.scLbl}>{lbl}</div><div style={{...s.scVal,color:col}}>{val}</div></div>
                ))}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 280px",gap:14}}>
                ...
                <div style={s.card}>
                  ...
                  {Object.entries(techs).map(([id,t])=>{
                    const mj=jobList.filter(([,j])=>(j.techIds||[]).includes(id)&&!["done","cancelled"].includes(j.status)).length;
                    return(<div key={id} style={s.techRow}><div style={{...s.techAv,background:t.bg,color:t.color}}>{t.initials}</div><div style={{flex:1}}><div style={{fontWeight:600,fontSize:12.5}}>{t.name}</div><div style={{fontSize:10,color:"#7b87b8"}}>{t.area}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:10.5,color:mj>0?"#f0a030":"#2dcc7a"}}>{mj>0?mj+" active":"Free"}</div><div style={{fontSize:9,color:"#3d4668",fontFamily:"monospace"}}>login: {t.loginName}</div></div></div>);
                  })}
                </div>
              </div>
            </div>
                ...
                {Object.entries(techs).map(([id,t])=>{
                  ...
                  return(<div key={id} style={{background:"#0c0f1a",border:"1px solid #222840",borderRadius:12,padding:16}}>
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                      <div style={{...s.techAv,width:40,height:40,fontSize:13,background:t.bg,color:t.color,cursor:"pointer"}} onClick={()=>setConfirmDeleteTechId(id)} title="Click to delete">{t.initials}</div>
      ...
      {showModal&&(
        <div style={s.modalOv} onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div style={s.modal}>
              ...
              isDone(editJobId)?(
                ...
                <div style={s.modalBody}>
                  <div style={{background:jobs[editJobId]?.status==="cancelled"?"#2a0a0a":"#081e13",border:"1px solid",borderColor:jobs[editJobId]?.status==="cancelled"?"#5a1a1a":"#1a5a2a",borderRadius:10,padding:"14px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}>
                    ...
                    <div>
                      ...
                      <div style={{fontSize:11,color:"#7b87b8",marginTop:3}}>{jobs[editJobId]?.status==="cancelled"?`Cancelled by ${jobs[editJobId]?.cancelledBy||"—"} · ${jobs[editJobId]?.cancelledAt?new Date(jobs[editJobId].cancelledAt).toLocaleString("en-PH"):""}`:`Completed by ${jobs[editJobId]?.updatedBy||getCurrentTechNames(jobs[editJobId]?.techIds)} · ${jobs[editJobId]?.updatedAt?new Date(jobs[editJobId].updatedAt).toLocaleString("en-PH"):""}`}</div>
                    </div>
                  </div>
                  ...
                  {jobs[editJobId]?.materialsUsed?.length>0&&(
                    <div style={{background:"#111525",border:"1px solid #222840",borderRadius:10,padding:"14px 16px",marginBottom:14}}>
                      ...
                      {jobs[editJobId].materialsUsed.map((m,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:i<jobs[editJobId].materialsUsed.length-1?"1px solid #222840":"none"}}><span style={{fontSize:13,color:"#dde3ff"}}>{m.name} <span style={{color:"#7b87b8"}}>x{m.qty}</span></span><span style={{fontFamily:"monospace",fontSize:13,fontWeight:700,color:"#2dcc7a"}}>₱{(m.price*m.qty).toLocaleString()}</span></div>))}
                  ...
                  {jobs[editJobId]?.itUsername&&(
                    <div style={{background:"#0d1535",border:"1px solid #4d8ef5",borderRadius:10,padding:"14px 16px"}}>
                      ...
                      <div style={{marginBottom:10}}><div style={{fontSize:9.5,color:"#7b87b8",marginBottom:4,textTransform:"uppercase",fontWeight:700}}>Username</div><div style={{fontFamily:"monospace",fontSize:14,fontWeight:700,color:"#4d8ef5",wordBreak:"break-all"}}>{jobs[editJobId]?.itUsername}</div></div>
                      <div><div style={{fontSize:9.5,color:"#7b87b8",marginBottom:4,textTransform:"uppercase",fontWeight:700}}>Password (MAC)</div><div style={{fontFamily:"monospace",fontSize:14,fontWeight:700,color:"#9b78f5",wordBreak:"break-all"}}>{jobs[editJobId]?.itPassword||jobs[editJobId]?.macAddress}</div></div>
                ...
                <div style={s.modalBody}>
                  ...
                  <FG label="Assign Technicians">
                    <TechCheckboxList techs={techs} form={form} setForm={setForm} todayAttendance={todayAttendance} techCanDeploy={techCanDeploy} ATTEND_STATUS_COLOR={ATTEND_STATUS_COLOR} ATTEND_STATUS_LABEL={ATTEND_STATUS_LABEL} ATTEND_STATUS_ICON={ATTEND_STATUS_ICON} s={s}/>
                  </FG>
              ...
              <div style={s.modalBody}>
                ...
                <div style={{background:"#0d1535",border:"1.5px solid #4d8ef5",borderRadius:12,padding:14,marginBottom:18}}>
                  ...
                  <div style={{marginTop:8,fontSize:10.5,color:"#7b87b8",lineHeight:1.5}}>
                    {parseMode==="labeled"
                      ?<><strong style={{color:"#dde3ff"}}>🏷 Labels:</strong> TASK:, Name:, Contact Number:, Address:, LCP NAP PORT:, ISSUE:</>
                      :<><strong style={{color:"#dde3ff"}}>🔓 Smart:</strong> Auto-detect phone (09XX...), LCP/NAP/PORT (L# N# P#), name, address, issue.</>}
                  </div>
                  {parsedOk&&(
                ...
                <div style={s.f2}>
                  <FG label="Contact"><input style={s.fi} value={form.contact} onChange={e=>setForm({...form,contact:e.target.value})} placeholder="09XXXXXXXXX"/></FG>
            ...
            <div style={s.modalFt}>
              <button style={s.btnGhost} onClick={()=>{setShowModal(false);setEditJobId(null);setForm(emptyForm());resetPaste();}}>{editJobId?"Close":"Cancel"}</button>
...
function TechCheckboxList({techs,form,setForm,todayAttendance,techCanDeploy,ATTEND_STATUS_COLOR,ATTEND_STATUS_LABEL,ATTEND_STATUS_ICON,s}){
  return(<><div style={{background:"#111525",border:"1px solid #222840",borderRadius:8,padding:"8px 11px",maxHeight:150,overflowY:"auto"}}>
    {Object.entries(techs).map(([id,t])=>{
  ...
  </div><div style={{fontSize:10.5,color:"#7b87b8",marginTop:5}}>⛔ = Hindi pwedeng i-assign (Absent/Leave/Day Off)</div></>);
}
...
const s={
  ...
  filterLabel:{fontSize:10,color:"#7b87b8",minWidth:40},
  ...
  dateInput:{background:"#111525",border:"1px solid #4d8ef5",color:"#dde3ff",padding:"4px 10px",borderRadius:5,fontFamily:"inherit",fontSize:11,outline:"none"},
