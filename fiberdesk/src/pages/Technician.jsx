...
export default function Technician({ user, onLogout }) {
  ...
  const [tab, setTab] = useState("dashboard"); // dashboard | tasks | history | profile
  const [historyView, setHistoryView] = useState("daily"); // daily | monthly
  ...
  const [installForm, setInstallForm] = useState({ realName:"", realAddress:"", realContact:"", realPlan:"", realReferral:"", lcp:"", nap:"", port:"", macAddress:"", modemSerial:"", techNotes:"" });
  const [cancelJobId, setCancelJobId] = useState(null);
  const [cancelReason, setCancelReason] = useState("");
  const [profileForm, setProfileForm] = useState({ name:"", contact:"", area:"", spec:"", profilePic:"" });
  ...
  // All my jobs
  const allMyJobs = Object.entries(jobs).filter(([,j]) => (j.techIds || []).includes(user.techId));
  ...
  async function submitCancel() {
    ...
    await update(ref(db,"jobs/"+cancelJobId), { status:"cancelled", cancelReason:cancelReason.trim(), cancelledBy:user.name, cancelledAt:new Date().toISOString(), updatedAt:new Date().toISOString() });
  ...
  function openInstallForm(jobId) {
    ...
    setUsedItems([]); setInstallJobId(jobId);
  }

  async function submitInstallDetails() {
    if (!installForm.macAddress.trim()) { alert("Kailangan ang MAC Address"); return; }
    if (!installForm.lcp.trim()||!installForm.nap.trim()||!installForm.port.trim()) { alert("Ilagay ang LCP, NAP, at PORT"); return; }
    const matTotal = usedItems.reduce((a,i) => a+(i.price*i.qty), 0);
    await update(ref(db,"jobs/"+installJobId), { ...installForm, status:"for-approval", macSubmittedAt:new Date().toISOString(), updatedBy:user.name, materialsUsed:usedItems, materialsTotal:matTotal });
  ...
  function changeQty(i, val) {
    const u = [...usedItems]; u[i].qty = Math.max(0, parseInt(val)||0); setUsedItems(u.filter(x=>x.qty>0));
  }

  async function submitDeclaration() {
    const total = usedItems.reduce((a,i) => a+(i.price*i.qty), 0);
    await update(ref(db,"jobs/"+declareJobId), { materialsUsed:usedItems, materialsTotal:total, status:"done", updatedAt:new Date().toISOString(), updatedBy:user.name });
    setDeclareJobId(null); setUsedItems([]); setConfirming(null);
  }

  async function submitProfile() {
    await update(ref(db,"technicians/"+user.techId), {
      name: profileForm.name.trim(),
      contact: profileForm.contact.trim(),
      area: profileForm.area.trim(),
      spec: profileForm.spec.trim(),
  ...
  return (
    <div style={s.app}>
      ...
      <div style={s.header}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:6,background:"#081e13",border:"1px solid #1a5a2a",borderRadius:20,padding:"4px 12px"}}>
            {user.profilePic ? (
              <img src={user.profilePic} alt="Profile" style={{width:24,height:24,borderRadius:"50%",border:"1px solid #2dcc7a",objectFit:"cover"}} onError={(e)=>e.target.style.display="none"}/>
            ) : (
              <span style={s.ldot}/>
            )}
          ...
          {myJobs.length>0 && (
            <div style={{background:"#f05555",color:"#fff",borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700,animation:"blink 2s infinite"}}>
      ...
      {/* BOTTOM NAV TABS */}
      <div style={s.tabBar}>
        {[["dashboard","◈","Dashboard"],["tasks","📋","Mga Task"],["history","📊","Records"],["profile","👤","Profile"]].map(([key,ic,lbl])=>(
          <button key={key} style={{...s.tabBtn,...(tab===key?s.tabActive:{})}} onClick={()=>setTab(key)}>
            <span style={{fontSize:16}}>{ic}</span>
            <span style={{fontSize:10,marginTop:2}}>{lbl}</span>
            {key==="tasks" && myJobs.length>0 && <span style={s.tabBadge}>{myJobs.length}</span>}
          </button>
        ))}
      </div>
        ...
        <div style={s.body}>
          <div style={s.greeting}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
              {user.profilePic ? (
                <img src={user.profilePic} alt="Profile" style={{width:50,height:50,borderRadius:"50%",border:"2px solid #4d8ef5",objectFit:"cover"}} onError={(e)=>e.target.style.display="none"}/>
              ) : (
                <div style={{width:50,height:50,borderRadius:"50%",background:"linear-gradient(135deg,#4d8ef5,#9b78f5)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:700,color:"#fff"}}>
                  {user.name.split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2)}
                </div>
              )}
              <div>
                <div style={s.greetName}>Kumusta, {user.name.split(" ")[0]}! 👋</div>
                <div style={s.greetSub}>{user.spec || "Technician"}</div>
              </div>
            </div>
              ...
              <div style={{display:"flex",alignItems:"flex-end",gap:8,height:120}}>
                {monthlyData.map((m,i)=>(
                  <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:5}}>
                    ...
                    <div style={{width:"100%",background:m.key===thisMonth?"#2dcc7a":"#4d8ef5",borderRadius:"4px 4px 0 0",height:m.count===0?4:Math.max(8,(m.count/maxCount)*90),opacity:m.key===thisMonth?1:0.55,transition:"height .4s"}}></div>
          ...
          <div style={s.sectionHd}>
            <span style={s.secTitle2}>TOTAL MATERIALS USED</span>
          </div>
        ...
        <div style={s.body}>
          {myJobs.length===0 ? (
            <div style={s.noTasks}>
              ...
              <div style={{fontSize:13,color:"#7b87b8",textAlign:"center"}}>Kapag may ibinaba sa iyo ang dispatcher, <br/>lalabas dito agad! MAG COBRA KA MUNA.🐍⚡</div>
            </div>
          ) : (
            <>
              ...
              {myJobs.map(([id,j]) => <TaskCard key={id} id={id} j={j} materials={materials} usedItems={usedItems} setUsedItems={setUsedItems} totalCost={totalCost} confirming={confirming} setConfirming={setConfirming} setCancelJobId={setCancelJobId} setCancelReason={setCancelReason} openInstallForm={openInstallForm} openDeclare={openDeclare} updateStatus={updateStatus} addItem={addItem} changeQty={changeQty}/>)}
            </>
          )}
        </div>
      )}
      ...
      {tab==="history" && (
        <div style={s.body}>
          ...
          {historyJobs.length===0 && (
            <div style={s.emptyCard}>Walang jobs para sa period na ito.</div>
          )}
        </div>
      )}
      ...
      {tab==="profile" && (
        <div style={s.body}>
          <div style={s.sectionHd}>
            <span style={s.secTitle2}>MY PROFILE</span>
            <button style={s.btnPrimary} onClick={()=>{setProfileForm({name:user.name||"",contact:user.contact||"",area:user.area||"",spec:user.spec||"",profilePic:user.profilePic||""});setShowProfileModal(true);}}>Edit Profile</button>
          </div>

          <div style={s.card}>
            <div style={{padding:"20px",textAlign:"center"}}>
              <div style={{width:100,height:100,borderRadius:"50%",background:user.profilePic?"url("+user.profilePic+")":"linear-gradient(135deg,#4d8ef5,#9b78f5)",backgroundSize:"cover",backgroundPosition:"center",margin:"0 auto 16px",border:"3px solid #222840",display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,fontWeight:700,color:"#fff"}}>
                {!user.profilePic && user.name.split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2)}
              </div>
              <div style={{fontSize:20,fontWeight:800,color:"#dde3ff",marginBottom:4}}>{user.name}</div>
              <div style={{fontSize:14,color:"#7b87b8",marginBottom:8}}>{user.spec}</div>
              <div style={{fontSize:12,color:"#4d8ef5",fontFamily:"monospace"}}>ID: {user.techId}</div>
            </div>
          </div>

          <div style={s.card}>
            <div style={s.cardHd}>
              <span style={s.secTitle2}>CONTACT INFO</span>
            </div>
            <div style={s.cardBody}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                <span style={{fontSize:16}}>📞</span>
                <div>
                  <div style={{fontSize:14,color:"#dde3ff"}}>{user.contact || "Not set"}</div>
                  <div style={{fontSize:11,color:"#7b87b8"}}>Contact Number</div>
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:16}}>📍</span>
                <div>
                  <div style={{fontSize:14,color:"#dde3ff"}}>{user.area || "Not set"}</div>
                  <div style={{fontSize:11,color:"#7b87b8"}}>Area / Location</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={s.card}>
            <div style={s.cardHd}>
              <span style={s.secTitle2}>ACCOUNT INFO</span>
            </div>
            <div style={s.cardBody}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                <span style={{fontSize:16}}>🔑</span>
                <div>
                  <div style={{fontSize:14,color:"#dde3ff",fontFamily:"monospace"}}>{user.loginName}</div>
                  <div style={{fontSize:11,color:"#7b87b8"}}>Login Name</div>
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:16}}>🆔</span>
                <div>
                  <div style={{fontSize:14,color:"#dde3ff",fontFamily:"monospace"}}>{user.techId}</div>
                  <div style={{fontSize:11,color:"#7b87b8"}}>Technician ID</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={s.ov}>
          <div style={{...s.modal,width:560,maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{padding:"16px 18px"}}>
              ...
              <Fsec>Network Info</Fsec>
              <div style={s.f3}>
                <FI label="LCP *" v={installForm.lcp} set={v=>setInstallForm({...installForm,lcp:v})} ph="L1" mono/>
                <FI label="NAP *" v={installForm.nap} set={v=>setInstallForm({...installForm,nap:v})} ph="N2" mono/>
                <FI label="Port *" v={installForm.port} set={v=>setInstallForm({...installForm,port:v})} ph="P14" mono/>
              </div>
      ...
      {showProfileModal && (
        <div style={s.ov}>
          <div style={{...s.modal,width:480}}>
            <div style={{padding:"16px 18px",borderBottom:"1px solid #222840"}}>
              <div style={{fontSize:16,fontWeight:800,color:"#dde3ff"}}>👤 Edit Profile</div>
              <div style={{fontSize:11,color:"#7b87b8",marginTop:2}}>Update your technician information</div>
            </div>
            <div style={{padding:"16px 18px"}}>
              <Fsec>Basic Info</Fsec>
              <FI label="Full Name *" v={profileForm.name} set={v=>setProfileForm({...profileForm,name:v})} ph="Juan dela Cruz"/>
              <FI label="Specialization" v={profileForm.spec} set={v=>setProfileForm({...profileForm,spec:v})} ph="FTTH Installation"/>
              <Fsec>Contact Info</Fsec>
              <FI label="Contact Number" v={profileForm.contact} set={v=>setProfileForm({...profileForm,contact:v})} ph="0917-XXX-XXXX" mono/>
              <FI label="Area / Location" v={profileForm.area} set={v=>setProfileForm({...profileForm,area:v})} ph="Socorro / Bancal"/>

            </div>
            <div style={{padding:"12px 18px",borderTop:"1px solid #222840",display:"flex",justifyContent:"flex-end",gap:8}}>
              <button style={s.btnGhost} onClick={()=>setShowProfileModal(false)}>Cancel</button>
              <button style={s.btnPrimary} onClick={submitProfile}>Save Changes</button>
            </div>
          </div>
        </div>
      )}
...
function TaskCard({ id, j, materials, usedItems, setUsedItems, totalCost, confirming, setConfirming, setCancelJobId, setCancelReason, openInstallForm, openDeclare, updateStatus, addItem, changeQty }) {
  ...
  const guide = { dispatched:"I-tap kapag umalis ka na.", "on-way":"I-tap kapag nandoon ka na.", "on-site":isInstall?"I-submit ang installation details at MAC Address.":"I-tap ang TAPOS NA kapag done ka.", "for-approval":"Naghihintay ng IT.", configuring:"Kina-configure ng IT ang modem.", activated:"Activated! I-tap ang Done.", pending:"Naghihintay ng dispatcher." };
  ...
  return (
    <div style={{...s.card,border:"1.5px solid "+borderColor,marginBottom:14}}>
      ...
      <div style={s.cardBody}>
        ...
        {j.lcp && !["for-approval","activated"].includes(j.status) && <div style={s.lcpBox}>LCP: {j.lcp} &nbsp;|&nbsp; NAP: {j.nap} &nbsp;|&nbsp; PORT: {j.port}</div>}
        {j.notes && <div style={s.notesBox}>⚠ {j.notes}</div>}
        <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>{actions}</div>
      </div>
...
function FI({ label, v, set, ph, mono, big, full, ta }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:12,gridColumn:full?"1/-1":"auto"}}>
      ...
      {ta?<textarea style={{...st,minHeight:70,resize:"vertical"}} value={v} onChange={e=>set(e.target.value)} placeholder={ph}/>:<input style={st} value={v} onChange={e=>set(e.target.value)} placeholder={ph}/>}
    </div>
  );
}
...
const s = {
  ...
  modal:{background:"#0c0f1a",border:"1px solid #2e3450",borderRadius:16,maxWidth:"96vw"},
